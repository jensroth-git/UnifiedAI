...
type AnthropicContentBlock =
    AnthropicToolUse|AnthropicToolResult|AnthropicTextBlock;
...
export class ClaudeModel extends BaseModel {
   ...
   */
  private async checkRateLimits(estimatedInputTokens: number = 1000):
      Promise<void> {
    const now = Date.now();
    ...
    if (ClaudeModel.rateLimitInfo.remainingTokens < LOW_TOKEN_THRESHOLD) {
      console.warn(
          `[CLAUDE] Low on remaining tokens: ${
              ClaudeModel.rateLimitInfo.remainingTokens} remaining. ` +
          `Reset at ${
              new Date(ClaudeModel.rateLimitInfo.resetTime).toISOString()}`);
    }

    // If we have enough tokens remaining, proceed
    if (ClaudeModel.rateLimitInfo.remainingTokens >
    ...
    if (ClaudeModel.rateLimitInfo.resetTime > 0) {
      ...
      if (waitTime > 0) {
        console.log(`Approaching rate limit. Waiting ${
            Math.ceil(waitTime / 1000)} seconds for limits to reset.`);
  ...
  private updateRateLimitInfo(headers: any): void {
    ...
    if (remaining !== undefined) {
      ...
      if (ClaudeModel.rateLimitInfo.remainingTokens < 5000) {
        console.warn(`[CLAUDE] Running low on tokens: ${
            ClaudeModel.rateLimitInfo.remainingTokens} remaining`);
      }
    }
    ...
    if (resetStr) {
      ...
      try {
        ...
        console.log(
            `[CLAUDE] Rate limits reset at: ${resetDate.toISOString()}`);
      } catch (e) {
  ...
  convertMessagesForClaude(messages?: BaseMessage[]):
      Anthropic.Messages.MessageParam[] {
    if (!messages || messages.length === 0) {
      return [];
    }

    // const toolUseMap = new Map<string, AnthropicToolUse>(); // Maps
    // tool_use_id to the tool_use message
    ...
    for (const message of messages) {
      ...
      // else if (message.role === 'assistant' && (message as
      // BaseToolCall).functionCall) { Store tool use information for pairing
      // with responses const toolCall = (message as BaseToolCall).functionCall;
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      if (message.role === 'system') {
        ...
        if ((message as BaseImageMessage).content) {
          ...
          claudeMessages.push({role: 'user', content: imageContents});
        } else if ((message as BaseTextMessage).text) {
          // Handle user messages
          let content: string|any[] = (message as BaseTextMessage).text;

          // For the first user message, prepend system messages in Claude's
          // format
        ...
        if ((message as BaseTextMessage).text) {
          // Simple text message
          claudeMessages.push(
              {role: 'assistant', content: (message as BaseTextMessage).text});
          ...
          claudeMessages.push({role: 'assistant', content: toolUseContent});
        }
      } else if (
          message.role === 'function' &&
          (message as BaseToolResponse).functionResponse) {
  ...
  convertMessagesFromClaude(
      messages: Anthropic.Messages.MessageParam[],
      toolNameMap: Map<string, string>): BaseMessage[] {
          ...
          for (const content of message.content) {
            if (content.type === 'tool_use') {
              baseMessages.push({
                role: 'assistant',
                functionCall:
                    {id: content.id, name: content.name, args: content.input}
              });
            } else if (content.type === 'text') {
              baseMessages.push({role: 'assistant', text: content.text});
            }
          }
        } else {
        ...
        if (Array.isArray(message.content)) {
          for (const content of message.content) {
            if (content.type === 'tool_result') {
              ...
              baseMessages.push({role: 'user', text: content.text});
            }
          }
        } else {
          baseMessages.push({role: 'user', text: message.content as string});
        }
      }
    }
  ...
  convertToolForClaude(name: string, zodTool: BaseTool):
  ...
  override async generateText(options: GenerateTextOptions):
    ...
    try {
      ...
      if (options.tools) {
        for (const key in options.tools) {
          const toolDefinition =
              this.convertToolForClaude(key, options.tools[key]);
        ...
        if (typeof cleanedResponse.content === 'string') {
          if (options.textMessageGenerated) {
            await options.textMessageGenerated(
                {role: 'assistant', text: cleanedResponse.content});
          }
        ...
        if (Array.isArray(cleanedResponse.content)) {
          ...
          for (const toolUse of toolUses) {
            ...
            if (options.tools && options.tools[toolUse.name]) {
              ...
              const toolResponse =
                  await tool.execute(toolUse.input, toolOptions);
              ...
              const toolResultMessage: Anthropic.Messages.MessageParam = {
                ...
                content: [{
                  ...
                  content:
                      toolResponse !== null ? JSON.stringify(toolResponse) : ''
                }]
              };
      ...
      const processedMessages =
          this.convertMessagesFromClaude(addedMessages, toolNameMap);

      let text = null;

      if (addedMessages.length > 0) {
        if (typeof addedMessages[addedMessages.length - 1].content ===
            'string') {
          text = addedMessages[addedMessages.length - 1].content;
        } else if (Array.isArray(
                       addedMessages[addedMessages.length - 1].content)) {
          text = ((addedMessages[addedMessages.length - 1].content as
      ...
      return {addedMessages: [], text: `Error: ${error}`};
    }
  }
}
